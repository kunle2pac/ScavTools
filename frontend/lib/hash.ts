// Real hash implementations for the Hash Generator tool.
// SHA-1/256/512 use the browser's Web Crypto API. MD5 isn't exposed by
// Web Crypto, so it's implemented directly (public-domain algorithm).
...
async function sha(algorithm: "SHA-1" | "SHA-256" | "SHA-512", input: string): Promise<string> {
...
function md5Raw(s: string): number[] {
  const n = s.length
...
function hexFromInt32(n: number): string {
